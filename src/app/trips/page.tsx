...
const categories = ["All", "Mountains", "Beaches", "Backpacking", "Winter", "International"];
    ...
    <main className="min-h-screen bg-gray-50">
      ...
      <div className="bg-[#0f172a] pt-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14 text-center">
          <p className="text-orange-500 font-semibold text-sm uppercase tracking-widest mb-3">Explore India & Beyond</p>
          <h1 className="text-4xl sm:text-5xl font-bold text-white mb-4">All Group Trips</h1>
          <p className="text-gray-400 text-lg max-w-xl mx-auto mb-8">
            Handcrafted itineraries with fixed departures, verified guides, and a community of 50,000+ travellers.
          </p>
          {/* Search */}
          <div className="max-w-xl mx-auto flex gap-3">
            ...
            <button
              ...
              className="flex items-center gap-2 bg-orange-500 hover:bg-orange-600 text-white px-4 py-3 rounded-xl font-semibold text-sm transition"
            >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* Filters */}
        {showFilters && (
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 mb-6 grid grid-cols-1 sm:grid-cols-3 gap-5">
            <div>
        ...
        <div className="flex gap-2 overflow-x-auto scrollbar-hide pb-3 mb-6">
          {categories.map((c) => (
            <button key={c} onClick={() => setCategory(c)}
              className={`shrink-0 px-5 py-2 rounded-full text-sm font-semibold transition ${category === c ? "bg-orange-500 text-white shadow-md" : "bg-white text-gray-600 border border-gray-200 hover:border-orange-300"}`}>
              {c}
            </button>
          ))}
        </div>
        ...
        <p className="text-gray-500 text-sm mb-6">
          Showing <strong className="text-gray-900">{filtered.length}</strong> trips
